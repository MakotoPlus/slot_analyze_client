...
interface ChartProps {
  ...
  height?: number;
  /** 指定時、この key の対象だけを描画する */
  filterKeys?: Set<string>;
}

export function PayoutTrendChart({ result, height = 280, filterKeys }: ChartProps) {
  const labels = result.days.map(mmdd);
  const series = filterKeys ? result.series.filter((s) => filterKeys.has(s.key)) : result.series;
...
export function GameTotalChart({ result, height = 280, filterKeys }: ChartProps) {
  const labels = result.days.map(mmdd);
  const series = filterKeys ? result.series.filter((s) => filterKeys.has(s.key)) : result.series;
